...
class Auth extends CI_Controller
{
    ...
    public function index()
    {
        ...
        if ($this->form_validation->run() == false) {
            $data['title'] = 'Kinerjaku User Login';
    ...
    public function registration()
    {
        ...
        $this->form_validation->set_rules('email', 'Email', 'required|trim|valid_email|is_unique[user.email]', [
            ...
            'valid_email' => 'Email harus di isi dengan benar!',
        ...
        if ($this->form_validation->run() == false) {
            $data['title'] = 'Kinerjaku User Registration';
            $this->load->view('templates/auth_header', $data);
    ...
    private function _sendEmail($token, $type)
    {
        $config = [
            ...
            'smtp_user' => 'vatanz.my@gmail.com',
            'smtp_pass' => 'sh4mp0k0ng',
            'smtp_port' => 465,
            'mailtype' => 'html',
            'charset' => 'utf-8',
            'newline' => "\r\n"
        ...
        $this->email->from('vatanz.my@gmail.com', 'kinerjaku.id');
        $this->email->to($this->input->post('email'));

        if ($type == 'verify') {
            $this->email->subject('Verifikasi Akun Kinerjaku');
            ...
            $this->email->subject('Reset Password Kinerjaku.id');
            $this->email->message('Klik link ini untuk reset password akun anda : <a href="' . base_url() . 'auth/resetpassword?email=' . $this->input->post('email') . '&token=' . urlencode($token) . '">Reset</a>');
        }
    ...
    public function logout()
    {
        ...
        $this->session->set_flashdata('message', '<div class=" alert alert-success" role="alert">Selamat! Akun anda berhasil di logout.</div>');
        redirect('auth');
    }
